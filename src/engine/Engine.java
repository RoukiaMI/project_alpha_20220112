   ...
   for (PhantomService p:data.getPhantoms()){
      if(
              (data.getHeroesPosition().x-p.getPosition().x)*(data.getHeroesPosition().x-p.getPosition().x)+
                ...
                // Appliquer méthode tuer phantom + score++
            boolean hit=true;
            data.setIsHint(hit);
            if(data.killPhantom(p,data.getPhantoms(),data.getIsHint())){
              data.setScoreH(data.getScoreH()+100);
            }

        //if(data.getIsHint()==false){// pas tiré
         // data.setIsHint(True);
        //}
      }
   }
